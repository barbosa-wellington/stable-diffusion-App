...
if __name__ == '__main__':
    ...
    # filename = os.path.basename(img_paths)
    # file_no_exist = os.path.splitext(filename)[0]
    with torch.no_grad():
        for img_path in img_paths:
            ...
            # fixing this file path to allow DSINE save the normal depth image ont the folder output
            target_path = img_path.replace("/img", "/output").replace(ext, '.png')
            # target_path = os.path.join('./samples/output/', file_no_exist + '.png')
            print(f"The output is being saving here: {target_path} ")
